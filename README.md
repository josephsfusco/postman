...
# Time Complexity
There are three areas we need to consider time complexity.  

### **User Lookup on Login**
* O(1) time complexity
  
Here we have all users stored in a dictionary `{'username' : encryptedPassword}` **our look up time is an instantanous O(1)** as all entries are indexed  

### **Token Lookup on Authentication**
* O(1)
  
Here we have all users stored in a dictionary `{'key' : Token(object)}` **our look up time is an instantanous O(1)** as all entries are indexed  

### **Token Cleanup**
* O(n)

While it could be faster this was a calculated trade off, we gave up background cleanup job speed to give our client facing token lookup on authentication a highly perfomrant O(1) time complexity. Although O(n) is not terrible it certainly could be better, by example this could be enhanced by keeping the tokens ordered by TTL and do a binary search to give us the first dead token and remove all of that below it.
